...
uint8_t SHTC3_Read_DATA() {
  ...
  state = !bcm2835_i2c_begin();
  state |=
      SHTC3_WriteCommand(SHTC3_NM_CD_ReadTH); // Read temperature first,clock
                                              // streching disabled (polling)
  bcm2835_delay(20);
  ...
  state |=
      SHTC3_WriteCommand(SHTC3_NM_CD_ReadRH); // Read temperature first,clock
                                              // streching disabled (polling)
  bcm2835_delay(20);
  ...
  if (!state) {
    TH_Value =
        175 * (float)TH_DATA / 65536.0f - 45.0f; // Calculate temperature value
